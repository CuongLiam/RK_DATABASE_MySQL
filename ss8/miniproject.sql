...
-- Câu 1: Liệt kê tên khách và số điện thoại của tất cả khách hàng
SELECT guest_name, phone
FROM guests;

-- Câu 2: Liệt kê các loại phòng khác nhau trong khách sạn
SELECT DISTINCT room_type
FROM rooms;

-- Câu 3: Hiển thị loại phòng và giá thuê theo ngày, sắp xếp theo giá tăng dần
SELECT room_type, price_per_day
FROM rooms
ORDER BY price_per_day ASC;

-- Câu 4: Hiển thị các phòng có giá thuê lớn hơn 1.000.000
SELECT *
FROM rooms
WHERE price_per_day > 1000000;

-- Câu 5: Liệt kê các lần đặt phòng diễn ra trong năm 2024
SELECT *
FROM bookings
WHERE YEAR(check_in) = 2024;

-- Câu 6: Cho biết số lượng phòng của từng loại phòng
...
-- Câu 7: Liệt kê danh sách các lần đặt phòng với tên khách, loại phòng, ngày nhận phòng
SELECT 
    g.guest_name AS ten_khach,
    r.room_type AS loai_phong,
    b.check_in AS ngay_nhan_phong
FROM bookings b
JOIN guests g ON b.guest_id = g.guest_id
JOIN rooms r ON b.room_id = r.room_id;

-- Câu 8: Cho biết mỗi khách đã đặt phòng bao nhiêu lần
SELECT 
    g.guest_name AS ten_khach,
...
-- Câu 9: Tính doanh thu của mỗi phòng
SELECT 
    r.room_id,
    r.room_type,
    r.price_per_day,
...
-- Câu 10: Hiển thị tổng doanh thu của từng loại phòng
SELECT 
    r.room_type,
...
-- Câu 11: Tìm những khách đã đặt phòng từ 2 lần trở lên
SELECT 
    g.guest_name AS ten_khach,
    COUNT(b.booking_id) AS so_lan_dat_phong
FROM guests g
...
-- Câu 12: Tìm loại phòng có số lượt đặt phòng nhiều nhất
SELECT 
    r.room_type,
    COUNT(b.booking_id) AS so_luot_dat_phong
FROM rooms r
...
-- Câu 13: Hiển thị những phòng có giá thuê cao hơn giá trung bình của tất cả các phòng
SELECT *
FROM rooms
WHERE price_per_day > (SELECT AVG(price_per_day) FROM rooms);

-- Câu 14: Hiển thị những khách chưa từng đặt phòng
SELECT *
FROM guests
WHERE guest_id NOT IN (SELECT DISTINCT guest_id FROM bookings);

-- Câu 15: Tìm phòng được đặt nhiều lần nhất
